...
const ShoppingCart: React.FC = () => {
  ...
  return (
    <div className="bg-white text-gray-500 font-sans">
      <header className="bg-cover bg-center h-48 flex items-center justify-center" style={{ backgroundImage: 'url(/path/to/header-bg.jpg)' }}>
        <h1 className="text-5xl font-bold text-gray-800 tracking-wide">Shopping Cart</h1>
      </header>
            ...
            {cartItems.map((item, index) => (
              <tr key={index} className="border-b">
                <td className="p-4 flex items-center">
                  <Image src={item.image} alt={item.name} height={93} width={93} className="w-16 h-16 object-cover rounded mr-4" />
                  <span>{item.name}</span>
                </td>
